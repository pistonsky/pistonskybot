...
class Bot:
        ...
        for user_id in self.users:
            for (sender_id, voice_id, ) in new_messages:
                if sender_id != user_id:
                    self.http_client.fetch("{url}/sendVoice".format(url=self.BASE_URL),
    ...
    def _run_once(self):
        new_messages = []
        response = self.http_client.fetch("{url}/getUpdates".format(url=self.BASE_URL),
            method='POST', body=fast_urlencode({
        ...
        if data['ok']:
            ...
            if len(new_messages) > 0:
                logging.info("Broadcasting {N} new audio messages...".format(N=len(new_messages)))
